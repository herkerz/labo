...
ksemilla_azar  <- c(112333,223444,334555,445666,556777)  #Aqui poner la propia semilla
kexperimento   <- "bayesiana_15_ajustado_ipc"
ktraining      <- c( 202101,
                     202012,
                     202011,
                     202010,
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  for (semilla in  ksemilla_azar) {
    ...
    param_variable  <- list(  early_stopping_rounds= as.integer(50 + 5/x$learning_rate),
                              ...
                              num_leaves = ceiling((1 / x$perc_min_data_in_leaf) * x$coverage),
                              learning_rate = x$learning_rate,
                              feature_fraction = x$feature_fraction
    )
    
    param_completo  <- c( param_basicos, param_variable )
    ...
    if (ganancia_semilla < 25000000 / kfolds) break
    
  }
  
  ganancia_normalizada  <-  ganancia_total / cantidad_semillas * kfolds     #normailizo la ganancia
  ...
  param_completo$coverage <- x$coverage
  param_completo$perc_min_data_in_leaf <- x$perc_min_data_in_leaf
...
dataset <- ajustar_valores(dataset)
